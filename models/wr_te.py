...
class WRTEModel:
    ...
    def fit(
        ...
        *,
        ...
        # Per-stat lock". dist_family="decomposed" routes receptions through
        # targets x catch_rate Monte Carlo composition (its strongest signal).
        # receiving_yards and receiving_tds automatically use the count_aware
        # quantile/NegBin paths since decomposition is only implemented for
        # receptions in this position model.
        if weekly is None:
            if use_weather:
        ...
        # Spread calibration — see models/qb.py.
        for stat in _TARGET_STATS:
            ...
            try:
                ...
                continue
            self._resid_cv[stat] = residual_cv(train_receivers[stat].values.astype(float), fitted)
    ...
    def predict(
        ...
        *,
        ...
        for stat in _TARGET_STATS:
            ...
            if future_row is not None and player_rows.empty:
                # Cold start (Week 1 / future season): blend the GLM point
                # estimate with the player's own trailing average, trusting the
                # GLM more as the prior-season sample grows. No usable history
                # falls back to the league prior. See models/qb.py.
